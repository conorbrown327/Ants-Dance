                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    ...
    Color black(0, 0, 0);
    Color body(197.0/255.0, 140.0/255.0, 133.0/255.0);
    Color red(1, 0, 0);
    Color green(0, 1, 0);
    Color blue(0, 0, 1);
    ...
    if (bone_name == "lhipjoint" || bone_name == "rhipjoint") {
        Matrix4 S = Matrix4::Scale(Vector3(0.005f, 0.15f, 0.005f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, blue);
    }
    if (bone_name == "lfemur" || bone_name == "rfemur") {
        Matrix4 S = Matrix4::Scale(Vector3(0.05f, 0.15f, 0.05f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, blue);
    }
    if (bone_name == "ltibia" || bone_name == "rtibia") {
        Matrix4 S = Matrix4::Scale(Vector3(0.05f, 0.3f, 0.05f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, blue);
    }
    if (bone_name == "lfoot" || bone_name == "rfoot") {
        Matrix4 S = Matrix4::Scale(Vector3(0.01f, 0.3f, 0.02f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, red);
    }
    if (bone_name == "ltoes" || bone_name == "rtoes") {
        Matrix4 S = Matrix4::Scale(Vector3(0.005f, 0.005f, 0.005f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, red);
    }
    if (bone_name == "lowerback") {
        Matrix4 S = Matrix4::Scale(Vector3(0.2f, 0.1f, 0.04f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, green);
    }
    ...
    if (bone_name == "thorax") {
        Matrix4 S = Matrix4::Scale(Vector3(0.02f, 0.1f, 0.04f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, green);
    }
    if (bone_name == "lowerneck" || bone_name == "upperneck") {
        Matrix4 S = Matrix4::Scale(Vector3(0.02f, 0.1f, 0.04f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, body);
    }
    if (bone_name == "head") {
        Matrix4 S = Matrix4::Scale(Vector3(0.1f, 0.1f, 0.1f));
        quick_shapes_.DrawSphere(ctm * S, view_matrix, proj_matrix, body);
    }
    ...
    if (bone_name == "lhumerus" || bone_name == "rhumerus" || bone_name == "lradius" || bone_name == "rradius") {
        Matrix4 S = Matrix4::Scale(Vector3(0.03f, 0.2f, 0.03f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, body);
    }
    if (bone_name == "lwrist" || bone_name == "rwrist") {
        Matrix4 S = Matrix4::Scale(Vector3(0.005f, 0.05f, 0.005f));
        quick_shapes_.DrawCube(ctm * S, view_matrix, proj_matrix, body);
    }
    ...
    Matrix4 child_root_transform = Matrix4::Translation(skeleton_.BoneDirectionAndLength(bone_name)) * ctm;
